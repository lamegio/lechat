...
import { NavigationMenu } from "@/types/siteConfig";
import CommonIcon from "@/components/ui/CommonIcon";

export default function Navigation({ siteNav }: { siteNav: NavigationMenu[] }) {
      ...
      {siteNav?.map((item) => (
        ...
        >
          ...
          >
            ...
            <CommonIcon type={item.icon} className="w-5 h-5" />
            {item.name}
          </Link>
              ...
              >
                {item.subMenu.map((subItem) => (
                  ...
                  >
                    ...
                    <CommonIcon type={subItem.icon} className="w-4 h-4" />
                    {subItem.name}
                  </Link>
                ))}
              </motion.div>
